...
class SoftmaxEstimateionModel(nn.Module):
...
def main():
    ...
    variables = SoftmaxEstimateionModel().init(key1, train_images[0:1])
    print(jax.tree_util.tree_map(lambda x: x.shape, variables['params']))

    state = train_state.TrainState.create(
        apply_fn=SoftmaxEstimateionModel().apply,
    ...
    fig, ax = plt.subplots()
    ax.plot(history['acc_train'], label='Accuracy (Train)')
    ax.plot(history['acc_test'], label='Accuracy (Test)')
    ax.set_xlabel('Epoch')
    ax.legend()
    plt.show()
    plt.close()
    ...
    fig, ax = plt.subplots()
    ax.plot(history['loss_train'], label='Loss (Train)')
    ax.plot(history['loss_test'], label='Loss (Test)')
    ax.set_xlabel('Epoch')
    ax.legend()
    plt.show()
    plt.close()
